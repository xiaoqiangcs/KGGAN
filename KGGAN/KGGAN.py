...
def run_training(args):
	...
	model = TransE(learning_rate=args.learning_rate,
				   ...
				   noise_dim =args.noise_dim,
					num_type = dataset.num_type,
				   entityid_to_typeid =dataset.entityid_to_typeid)
	...
	with graph_transe.as_default():
		...
		with tf.variable_scope("input"):
			...
			#id_triplets_negative = tf.placeholder(dtype=tf.int64, shape=[model.batch_size, num_negative*3], name='triplets_negative')
		...
		with tf.variable_scope("embedding"):
			embedding_entity = tf.get_variable(name='dis_entity', initializer=tf.random_uniform(
				shape=[dataset.num_entity, model.embedding_dimension], minval=-bound, maxval=bound))
			embedding_relation = tf.get_variable(name='dis_relation', initializer=tf.random_uniform(
	...
	with tf.Session(graph=graph_transe) as sess:
		...
		for epoch in range(model.num_epoch):
			...
			for batch in range(num_batch):
				...
				batch_positive,type_y, y_dim = dataset.next_batch_train(model.batch_size,num_negative)
				...
				if (batch + 1) % 100 == 0 or (batch + 1) == num_batch:
					print('epoch {}, batch {}, g_loss: {}, d_loss: {}'.format(epoch, batch, g_loss_batch, d_loss_batch))
			end_train = time.time()
			print('epoch {},mean batch g_loss: {:.3f}, ,mean batch d_loss: {:.3f},time elapsed last epoch: {:.3f}s'.format(epoch, g_loss_epoch / num_batch, d_loss_epoch / num_batch,end_train - start_train))
