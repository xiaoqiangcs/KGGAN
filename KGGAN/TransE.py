...
class TransE(object):
	...
	def generator(self,noise_Z):
		...
		G_W1 = tf.Variable(self.variable_init([self.num_negative * self.noise_dim, self.num_negative * 64]),dtype=tf.float32)
		G_b1 = tf.Variable(tf.zeros(shape=[self.num_negative * 64]), dtype=tf.float32)
		G_W2 = tf.Variable(self.variable_init([self.num_negative * 64, self.num_negative * self.embedding_dimension]),dtype=tf.float32)
	...
	def discriminator(self, id_triplets, postive, eps=1e-20):
		...
		# 输出G_log_prob是构建损失函数
		D_log_loss = tf.reduce_sum(D_log_prob,axis=0)
		return D_log_loss
	...
	def get_negative_embedding(self, G_prob, num_entity):
		...
		num_index = self.num_negative // 20
		embedding_entity_extend = tf.tile(embedding_entity,[num_index,1])
		for batch_index in range(self.batch_size):
			generator_index_samples = generator_samples[batch_index, :]
			for nagative_index in range(20):
				generator_nagative_index = generator_index_samples[nagative_index*num_index:(nagative_index+1)*num_index]
				generator_nagative_samples_extend = tf.reshape(tf.tile(generator_nagative_index,[1,num_entity]),[num_entity*num_index,self.embedding_dimension])
				if self.dissimilarity == 'L2':
					dissimilarity = tf.sqrt(tf.reduce_sum(tf.square(generator_nagative_samples_extend - embedding_entity_extend), axis=1))
				else:  # default: L1
					dissimilarity = tf.reduce_sum(tf.abs(generator_nagative_samples_extend - embedding_entity_extend), axis=1)
				dissimilarity_extend = tf.reshape(dissimilarity, shape=[num_index, num_entity])
				batch_negative.append(tf.argmin(dissimilarity_extend,axis=1))
				loss_sum += tf.reduce_sum(tf.reduce_min(dissimilarity_extend,axis=1))
		batch_negative = tf.reshape(batch_negative,shape=[self.batch_size,self.num_negative])
		return batch_negative,loss_sum

	def get_negative_sampling(self, batch_negative, id_triplets_positive):
		...
		index_positive= tf.Variable([1,2])
		batch_negative_sample = tf.concat([batch_negative_extend,tf.gather(triplets_positive,index_positive,axis=1)],axis=1)
	...
	def thumbel_softmax(self, G_prob, num_entity):
		# 根据生成的实体，从图中获取离该节点最近的点的距离，并把该样例作为负样例get embedding from the graph
		batch_negative = []  # 定义一个存储负样例的矩阵，矩阵大小为(batch_size *(num_negative*3))
		generator_samples= tf.reshape(G_prob,shape=[self.batch_size,self.num_negative,self.embedding_dimension])
		loss_sum = 0.0
