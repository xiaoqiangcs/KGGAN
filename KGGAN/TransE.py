...
class TransE(object):
    ...
    def generator(self,noise_Z):
        ...
        G_W1 = tf.Variable(self.variable_init([self.num_negative * self.noise_dim, self.num_negative * 64]),
                           name="gen_w1", dtype=tf.float32)
        G_b1 = tf.Variable(tf.zeros(shape=[self.num_negative * 64]), name="gen_b1", dtype=tf.float32)
        G_W2 = tf.Variable(self.variable_init([self.num_negative * 64, self.num_negative * 128]),
                           name="gen_w2", dtype=tf.float32)
        G_b2 = tf.Variable(tf.zeros(shape=[self.num_negative * 128]), name="gen_b2", dtype=tf.float32)

        G_W3 = tf.Variable(self.variable_init([self.num_negative * 128, self.num_negative * self.embedding_dimension]),name="gen_w3",dtype=tf.float32)
        G_b3 = tf.Variable(tf.zeros(shape=[self.num_negative * self.embedding_dimension]), name="gen_b3",dtype=tf.float32)
